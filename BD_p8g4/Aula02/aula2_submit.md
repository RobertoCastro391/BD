...
CodigoPagamento: Codigo, Nome, Prazo    
PK: Codigo      
FK(s): (none)

Fornecedor: (Codigo, Nome, Morada, NIF, CondiçoesPagamento_Codigo, TipoFornecedor_Codigo)        
PK: Codigo      
FK(s): CondiçoesPagamento_Codigo, TipoFornecedor_Codigo

Armazem: Codigo, Morada         
PK: Codigo          
FK(s): (none)

Produto: Codigo, Nome, Preço, IVA, Armazem_Codigo               
PK: Codigo          
FK(s): Armazem_Codigo       

Encomenda: Numero, Data, Fornecedor_Codigo, Produto_Codigo, Quantidade                    
PK: Numero          
...
```
No Sistema de Gestão de Stocks apresentado, conseguimos identificar 5 relações Binárias, onde sabemos que um 
armazem contém vários produtos (1:N), vários produtos podem estar inseridos em várias encomendas (N:M), por conseguinte 
um fornecedor tem várias encomendas (1:N). No que toca aos fornecedores é lhes atribuido um tipo, ou seja, 
vários fornecedores podem ter o mesmo tipo (N:1) e várias condições de pagamento podem ser aceites por vários 
fornecedores (M:N).
No que toca à obrigatoriedade podemos concluir que um produto está sempre num armazem e que qualquer que seja
a encomenda, esta contém sempre pelo menos um produto. As encomendas necessitam de ser processadas sempre por 
um fornecedor, que deve deter sempre a informação sobre o seu tipo de forncedor e as condições de pagamento.
```
